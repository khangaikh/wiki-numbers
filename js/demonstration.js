...
$(document).ready(function() {
	...
	if($(".usertable").length > 0){
		...
		oTable.columnFilter({
			...
			null,
			{
				type: "select",
				bCaseSensitive:true,
				values: ['Шинэ', 'Байнгын', 'Disabled', 'Бэлэн', 'Карт', 'Данс']
			},
			{
				type: "text",
			},
	...
	if($(".usertable1").length > 0){
		...
		var oTable = $('.usertable1').dataTable(opt2);
	...
	if($(".customertable").length > 0){
		var opt1 = {
	        ...
	        "footerCallback": function (row, data, start, end, display) {
	            var api = this.api(), data;
                    ...
                    // Update footer
                    $(api.column('.sum1').footer()).html(pageTotal);
	        },
			"iDisplayLength" : 50,
			"oLanguage":{
				"sSearch": "<span>Хайлт:</span> ",
		...
		oTable.columnFilter({
			...
			},
			{
				type: "select",
				bCaseSensitive:true,
				values: ['New','Confirmed','No-show','Checked-in','Checked-out','Cancelled']
			},
			{
				type: "select",
				bCaseSensitive:true,
				values: ['Updated','No-change']
			}
			]
		});
